...
This is to record the frame of the probe_tip on the apex point
```
...
```
This one is to record directly the apex frame, better
```
...
ROS_NAMESPACE=/iiwa python3 ~/iiwa_stack_ws/src/us_planner/scripts/us_pose_planner_confidence.py \
...
python3 record_us_best_pose_imfusion_csv.py \
  ...
  _flush_every:=1
```
To record the probe_tip on the best pose:
```
rosrun us_planner export_probe_pose_Tfixed_for_imfusion.py \
  _probe_frame:=probe_tip \
  _base_frame:=world \
